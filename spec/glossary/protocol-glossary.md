...
The protocol-visible correlation identifier (the `requestId` field) that links a logical control-plane request to its immediate responses. The request identifier may be supplied by the caller or generated by the implementation and should remain stable across retries or follow-on processing when the implementation intends them to represent the same logical request attempt. Transport bindings may carry it in headers or other metadata, but the JSON body field remains canonical for protocol purposes.
...
The transport-neutral wrapper for a control-plane request, carrying operation name, request identifier for correlation and retry idempotency, actor identity, and operation-specific payload. The request envelope structure preserves mandatory contract fields across all transport bindings while allowing binding-specific encoding (e.g., HTTP headers); the JSON body remains canonical for protocol purposes.

### Response Envelope

The transport-neutral wrapper for a control-plane response, carrying operation name, request identifier (for correlation with the request), and either a success result or a structured error object. A success response envelope contains an operation-specific result payload; an error response envelope contains an error object with code, category, message, and retryable flag. Both variants preserve the operation and requestId fields to maintain trace continuity.
...
A structured interoperable metadata document announcing a readiness profile's identity (`profileId`, `stewardId`, `revision`), support state (`supportState`), and declarative requirements such as base conformance and additional operational expectations. A profile declaration takes one of three forms — supported (profile fully implemented), standard (planned, deprecated, or replaced), or adopting (progressive adoption toward a target profile) — and is layered above baseline conformance rather than redefining protocol semantics.
...
A named string identifier anchoring a readiness profile declaration to a protocol-level conformance baseline upon which additional operational expectations are layered. Base conformance names the baseline protocol level (such as `baseline`) that the profile preserves and builds upon; a profile declaration should identify its base conformance to signal that it does not redefine or weaken the baseline semantics.
